...
namespace VeditorGP
{
    class VideoFunctions
    {
        ...
        public Frame InitialSegmentationBinaryFrame, InitialFrame;
        int WindowWidth = 30, WindowHeight = 30, WindowSize = 30 * 30;
        Capture Video;
        ...
        public void SetInitialWindowsArroundContour()
        {
            ...
            InitialFrame.FrameWindows.Add(new Window(WindowWidth, WindowHeight, InitialFrame, InitialSegmentationBinaryFrame, ConnectedContour[0]));
            int OverLappingArea, newIndex = 0, index = 0;

            OverLappingArea = WindowSize / 3;
            double Distance, Temp;
            int LoopCounter = 1, WindowCount = ConnectedContour.Count / (WindowSize - OverLappingArea);
            while (WindowCount > 0)
            {
                Distance = 0;
                ...
                {
                    Temp = Distance;
                    Distance = Math.Sqrt(Math.Pow((ConnectedContour[i].X - ConnectedContour[index].X), 2) + Math.Pow((ConnectedContour[i].Y - ConnectedContour[index].Y), 2));
                    if (Math.Floor(Distance) > 20)
                    {
                        newIndex = ConnectedContour.IndexOf(ConnectedContour[i - 1]);
                        LoopCounter = ++i;
                        WindowCount--;
                        InitialFrame.FrameWindows.Add(new Window(WindowWidth, WindowHeight, InitialFrame, InitialSegmentationBinaryFrame, ConnectedContour[newIndex]));
                        index = newIndex;
                        break;
                    }
                }
        ...
        public void TrainClassifiers()
        {
            ...
            //    item.WindowClassifier.Train(item);
        }
        #endregion
    }
}
