...
namespace GraduationProject
{
    ...
    {
        public SIFT() { }
        public Bitmap GetSIFTpoints(Frame F1, Frame F2)
        {
            MCvSURFParams surfParam = new MCvSURFParams(500, false);

            //Image<Gray, Byte> modelImage = new Image<Gray, byte>("box.png");
            Image<Gray, Byte> modelImage = F1.RgbImage.Convert<Gray, Byte>();
            //extract features from the object image
            SURFFeature[] modelFeatures = modelImage.ExtractSURF(ref surfParam);

            //Image<Gray, Byte> observedImage = new Image<Gray, byte>("box_in_scene.png");
            Image<Gray, Byte> observedImage = F2.RgbImage.Convert<Gray, Byte>();
